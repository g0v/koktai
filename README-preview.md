...
- 從 `.dic` 重產 `pug/`：**`bun run gen:pug`**（**純 TypeScript** pipeline `lib/dic/pipeline.ts`：CP950 recode（`cp950.ts`，含 Big5 造字區 → U+F0000 astral）→ 辭典 analyse（`dic2pug.ts`）／附錄 `txt2pug` → jade-unescape ＋ `finalizePugDocument`（`unescape.ts`）；**不** spawn Perl 或 Python，已與歷史 Perl 產線驗證 byte-identical）。對照已提交成品：**`bun run diff:pug`**（26 卷＋附錄，全文 `===`）。型別檢查：**`bun run check`**（`tsgo`）。legacy oracle：**`bun run parity:stage <recode|analyse|unescape>`** 會 spawn 舊 Perl／Python 腳本重驗移植（需要 perl＋`JSON`＋`File::Slurp` 與 python3）；`a-tsioh_sandbox/recode_utf8.pl`、`font/jade-unescape.pl`、`scripts/legacy-py3/` 保留原位僅供此用；僅 **`archive/gen.pl`**（舊 py2 Makefile 列印指令）已封存。
