...
describe("structured volume view model", () => {
  test(
    "volume 09: metadata, sections, and 【個人】 我 readings",
    () => {
  ...
  test(
    "volume 01: 【八荒】 preserves variant group alternatives",
    () => {
  ...
  test(
    "volume 01 ㄅㄚ section includes sinogram 八 before word entries",
    () => {
      const vol = getStructuredVolume(root, "01");
  ...
  test(
    "volume 01 preserves bitmap PUA sinogram head glyphs",
    () => {
      const vol = getStructuredVolume(root, "01");
